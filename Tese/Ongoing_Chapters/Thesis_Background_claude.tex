%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                      %
%     File: Thesis_Background_claude.tex                               %
%     Tex Master: Thesis.tex                                           %
%                                                                      %
%     Author: Andre C. Marta                                           %
%     Last modified :  4 Mar 2024                                      %
%                                                                      %
%     NOTE: Restructured/extended copy of Thesis_Background.tex.       %
%           Existing prose preserved verbatim; only section levels     %
%           and ordering were changed, plus new sections added         %
%           (Atmospheric Models, pseudo-forces, Numerical Methods)     %
%           and three previously-commented blocks reinstated.          %
%                                                                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Reference Frames and Earth Rotation}
\label{sec:reference_frames}

\subsection{Rotating Earth Frame and Inertial Frame}
\label{ssec:frames}
...
For the planar ascent state $[s,r,v,\gamma]$ this vector relation reduces to a set of scalar components. Resolving the rotating-frame velocity into local East--North--Up (ENU) components at latitude $\phi$ and heading $\psi$, the horizontal speed is $v_h = v\cos\gamma$, so that
\begin{subequations}\label{eq:ecef_eci}
\begin{align}
v_N &= v_h\cos\psi \ , \qquad v_E^{\mathrm{ECEF}} = v_h\sin\psi \ , \label{eq:ecef_eci_enu}\\[3pt]
v_E^{\mathrm{ECI}} &= v_E^{\mathrm{ECEF}} + \omega_E\,r\cos\phi \ , \label{eq:ecef_eci_add}\\[3pt]
v_{\mathrm{ECI}} &= \sqrt{\bigl(v_E^{\mathrm{ECI}}\bigr)^{2} + v_N^{2} + \bigl(v\sin\gamma\bigr)^{2}} \ , \label{eq:ecef_eci_speed}\\[3pt]
\gamma_{\mathrm{ECI}} &= \operatorname{atan2}\!\Bigl(v\sin\gamma,\; \sqrt{\bigl(v_E^{\mathrm{ECI}}\bigr)^{2} + v_N^{2}}\Bigr) \ , \label{eq:ecef_eci_gamma}
\end{align}
\end{subequations}
where only the eastward component is augmented by the surface rotation speed $\omega_E r\cos\phi$~\cite{relativityframes,curtis2020orbital}. Equations~\eqref{eq:ecef_eci} are the conversion actually applied in the simulator whenever orbital elements are evaluated (Chapter~\ref{chapter:methodology}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Rotating-Frame Pseudo-Forces}
\label{ssec:pseudo_forces}

Because the ascent dynamics in Eqs.~\eqref{eq:planar_eom} are most naturally expressed in the rotating Earth frame, the non-inertial nature of that frame must be accounted for through two apparent (pseudo) accelerations: the Coriolis and the centrifugal terms~\cite{vallado2013fundamentals}. These do not arise from any physical interaction, but from the rotation of the reference frame at angular rate $\boldsymbol{\omega}_E$, and they must be added to the equations of motion whenever Earth rotation is modelled~\cite{curtis2020orbital}.

For a vehicle moving with velocity $\mathbf{v}_{\mathrm{rot}}$ relative to the rotating frame at position $\mathbf{r}$, the Coriolis and centrifugal accelerations are~\cite{vallado2013fundamentals}:
\begin{equation}
\mathbf{a}_{\mathrm{Cor}} = -2\,\boldsymbol{\omega}_E\times\mathbf{v}_{\mathrm{rot}} \ ,
\qquad
\mathbf{a}_{\mathrm{cen}} = -\,\boldsymbol{\omega}_E\times\bigl(\boldsymbol{\omega}_E\times\mathbf{r}\bigr) \ .
\label{eq:pseudo_forces}
\end{equation}
The Coriolis term depends on the vehicle velocity and deflects the trajectory laterally, coupling the in-plane motion to the cross-range direction, whereas the centrifugal term depends only on position and acts to reduce the effective gravity, most strongly near the equator~\cite{curtis2020orbital}. The centrifugal contribution is small but non-negligible, while the Coriolis contribution is the dominant rotational effect on the achieved orbital inclination~\cite{vallado2013fundamentals}.

Evaluating the cross products in the local East--North--Up (ENU) basis at latitude $\phi$ gives the component form used in this work. With rotating-frame velocity components $(v_E,v_N,v_{\mathrm{up}})$, the Coriolis acceleration is~\cite{vallado2013fundamentals}
\begin{subequations}\label{eq:coriolis_enu}
\begin{align}
a_E^{\mathrm{cor}} &= -2\omega_E\cos\phi\;v_{\mathrm{up}} + 2\omega_E\sin\phi\;v_N \ , \\
a_N^{\mathrm{cor}} &= -2\omega_E\sin\phi\;v_E \ , \\
a_{\mathrm{up}}^{\mathrm{cor}} &= 2\omega_E\cos\phi\;v_E \ ,
\end{align}
\end{subequations}
and the centrifugal acceleration, which depends on position only, is~\cite{vallado2013fundamentals}
\begin{subequations}\label{eq:centrifugal_enu}
\begin{align}
a_E^{\mathrm{cen}} &= 0 \ , \\
a_N^{\mathrm{cen}} &= -\omega_E^{2}\,r\,\sin\phi\cos\phi \ , \\
a_{\mathrm{up}}^{\mathrm{cen}} &= \omega_E^{2}\,r\,\cos^{2}\!\phi \ .
\end{align}
\end{subequations}
Summing the two contributions in each direction ($a_E = a_E^{\mathrm{cor}}+a_E^{\mathrm{cen}}$, and likewise for $N$ and up) and projecting onto the along-heading horizontal and vertical directions gives
\begin{equation}
a_{h,\parallel} = a_E\sin\psi + a_N\cos\psi \ , \qquad
a_{h,\perp}     = a_E\cos\psi - a_N\sin\psi \ ,
\label{eq:pseudo_projection}
\end{equation}
with $a_{\mathrm{up}}$ retained for the radial direction. The along-heading and vertical components $a_{h,\parallel}$ and $a_{\mathrm{up}}$ enter the velocity and flight-path-angle rates of Eqs.~\eqref{eq:planar_eom_a}--\eqref{eq:planar_eom_b} as the additive corrections applied in the simulator (Chapter~\ref{chapter:methodology}), while the cross-heading term $a_{h,\perp}$ measures the lateral (inclination-changing) forcing.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Launch Azimuth and Orbit Inclination}
\label{ssec:launch_azimuth}
...
\end{equation}
This means that an azimuth angle, $A$, can only be selected if the launch-site latitude, $\lambda$, is smaller than the required orbital inclination $i$. Otherwise, reaching the target orbital plane requires an expensive out-of-plane manoeuvre~\cite{teofilatto2025trajectories}. Equation~\eqref{eq:azimuth} is the \emph{inertial} azimuth $A_I$: it orients the orbital plane in inertial space and admits a symmetric descending-node solution $A_I^{(\mathrm{desc})} = \pi - A_I$~\cite{Edberg}.

Because the launch site already carries an eastward inertial velocity due to Earth rotation, the heading that the guidance law must command \emph{relative to the rotating Earth} differs from $A_I$. At latitude $\phi$ this surface speed is $v_E = \omega_E R_E\cos\phi$~\cite{Edberg}. Writing the required inertial velocity as the sum of the ground-relative velocity and the site rotation, and matching the East and North components, the ground-relative launch azimuth is~\cite{BateMuellerWhite1971Fundamentals,curtis2020orbital}
\begin{equation}
A_G = \operatorname{atan2}\!\bigl(V_I\sin A_I - \omega_E R_E\cos\phi,\; V_I\cos A_I\bigr) \ ,
\label{eq:ground_azimuth}
\end{equation}
where $V_I$ is the inertial injection speed (for a circular orbit of radius $r$, $V_I=\sqrt{\mu/r}$). Earth rotation therefore does not change the required inclination itself; it only changes the ground-relative heading needed to realise the same inertial injection~\cite{curtis2020orbital}. The simulator uses the corrected azimuth $A_G$ by default and the geometric value $A_I$ as an option.

Conversely, the inclination \emph{achieved} at the end of the ascent is recovered from the inertial velocity of Eqs.~\eqref{eq:ecef_eci}: with the inertial velocity azimuth $\beta_{\mathrm{ECI}} = \operatorname{atan2}\!\bigl(v_E^{\mathrm{ECI}},\,v_N\bigr)$, the classical relation gives~\cite{curtis2020orbital}
\begin{equation}
\cos i_{\mathrm{achieved}} = \cos\phi\,\sin\beta_{\mathrm{ECI}} \ ,
\label{eq:achieved_inclination}
\end{equation}
so that the inclination drift $\Delta i = i_{\mathrm{achieved}} - i_{\mathrm{target}}$ serves as a fidelity metric for the rotating-frame treatment.
...
The aerodynamic forces introduced in Section~\ref{sec:External_Forces_Moments} depend directly on the local atmospheric density $\rho$, which varies by several orders of magnitude over the course of the ascent. A complete description is provided by tabulated standard atmospheres, such as the U.S. Standard Atmosphere~\cite{usstandard1976}, which model temperature, pressure, and density as piecewise functions of altitude across distinct atmospheric layers.

For preliminary trajectory analysis, however, it is common and sufficient to adopt a single-layer isothermal approximation in which density decays exponentially with altitude~\cite{curtis2020orbital}:
\begin{equation}
\rho(h) = \rho_0\,\exp\!\left(-\frac{h}{H}\right) \ ,
\label{eq:exp_atmosphere}
\end{equation}
where $h = r - R_E$ is the altitude above the surface, $\rho_0$ is the sea-level density, and $H$ is the density scale height, i.e.\ the altitude increment over which the density drops by a factor of $e$. Representative values for Earth are $\rho_0 = 1.225~\mathrm{kg/m^3}$ and $H \approx 8.5~\mathrm{km}$~\cite{curtis2020orbital}.

This exponential model captures the rapid thinning of the atmosphere that governs both the drag losses and the location of the maximum dynamic pressure (Eq.~\eqref{eq:q_dyn_press}), while remaining smooth and inexpensive to evaluate at every integration step. Its main limitation is that it neglects the temperature-driven layering of the real atmosphere; nevertheless, for ascent trajectory design the dominant effect is the overall density decay with altitude, which the model reproduces well~\cite{Ulrich}.
...
\subsection{Steering Laws during Atmospheric Arc}
...
\subsubsection{Pitch Manoeuvrer}
...
\subsubsection{Constant Pitch Rate (CPR) and Constant Flight Path Angle Rate (CFPAR)}

Although the gravity turn provides a simple and effective way to transition from near-vertical ascent to near-horizontal flight while maintaining a small angle of attack, it is not the only available option. In practice, alternative steering laws are often adopted to better trade drag losses, structural loads, and operational constraints. Two commonly used families are the \emph{constant pitch rate} (CPR) and the \emph{constant flight-path angle rate} (CFPAR) approaches~\cite{Ulrich}.
...
\subsection{Explicit Guidance Schemes}
...
\subsubsection{Polynomial Guidance Law}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Iterative Guidance Mode}
...
\subsubsection{Power Explicit Law}
\label{sssec:peg_theory}
...
Two PEG variants are implemented in this work. A classical variant parametrizes the pitch directly as a \emph{scalar} linear-tangent programme, $\sin(\text{pitch})=A+B\,t$, and refines its steering constants and burn-time estimate through a damped Guide--Estimate major loop; the analytical predictor--corrector variant instead carries the velocity-to-be-gained vector $\vec v_{go}$ derived above. These correspond respectively to the \texttt{peg} and \texttt{peg\_new} guidance modes, whose implemented forms are given in the methodology (Sections~\ref{ssec:peg} and~\ref{ssec:peg_new}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Exponential Pitch (Shooting) Law}

A further open-loop option parametrizes the commanded pitch programme directly as an exponential function of time, $\theta(t)=a\,e^{\,b\,t}$, so that the angle of attack follows as $\alpha(t)=\theta(t)-\gamma(t)$. The two coefficients $(a,b)$ are not tuned heuristically but determined once, at the start of the burn, by solving a small boundary-value problem: a simplified (drag-free, planar) forward propagation is required to terminate at the desired orbital radius with zero flight-path angle, and the resulting two residual equations are solved for $(a,b)$ by a Newton-type root finder~\cite{BettsSurvey1998}. This is a single-shooting approach (Section~\ref{ssec:rootfinding}), in which the open-loop steering parameters play the role of the unknowns adjusted until the terminal constraints are met. Once solved, the coefficients are held fixed for the remainder of the burn, so the law is purely open-loop.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/AscentPhasesUlrich.png}
    \caption{The three ascent phases. Source:~\cite{Ulrich}.}
    \label{fig:AscentPhasesUlrich}
\end{figure}
...
In this work an indirect method is adopted. Although indirect formulations are generally harder to initialise than direct ones, they provide more accurate solutions, which is desirable for the precise orbital-insertion requirements of launch-vehicle ascent~\cite{conway2010spacecraft}. The characteristic difficulty of the indirect approach---supplying good initial guesses for the adjoint variables---is overcome here by using a particle swarm optimiser to search the initial costates together with the trajectory timing parameters while enforcing the optimality conditions, as detailed in Sections~\ref{ssec:indirect_ascent} and~\ref{ssec:optimization}~\cite{pontani2014particle}.
...
\begin{aligned}
\lambda^{T}(t_f) &= \left.\bar{\Phi}_{x}\right|_{t=t_f}, \\
\left.(\bar{\Phi}_{t}+H)\right|_{t=t_f} &= 0.
\end{aligned}
\end{equation}
...
\subsection{Indirect Optimization of the Ascent Trajectory}
\label{ssec:indirect_ascent}

The indirect framework introduced above is now specialised to the powered-ascent problem addressed in this work, following the formulation of~\cite{Morgado2022_MultistagePrelimDesignTrajectoryOpt}. Direct methods are generally more robust in convergence, even with poor initial guesses, whereas the indirect method is harder to initialise but yields more accurate results, a property that is critical for aerospace applications~\cite{conway2010spacecraft}. In the baseline formulation the indirect control law is applied only on the free-flight (exo-atmospheric) arcs, after fairing jettison, where aerodynamic forces are negligible; during the preceding atmospheric flight the vehicle performs an uncontrolled gravity turn following the initial pitch manoeuvre. This assumption is relaxed by an optional full-ascent extension (Sections~\ref{sssec:fa_drag}--\ref{sssec:fa_masscostate}), which carries the same costate-based control through the atmospheric first-stage arc by augmenting the adjoint equations with aerodynamic drag and bounding the angle of attack.

Because the propellant mass flow is taken as constant, minimising the propellant consumption is equivalent to minimising the powered-flight duration. The objective is therefore the total impulse (thrusting) time required for orbital injection~\cite{Morgado2022_MultistagePrelimDesignTrajectoryOpt}:
\begin{equation}
J = t_f - t_{cf} \ ,
\label{eq:indirect_objective}
\end{equation}
where $t_f$ is the final time and $t_{cf}$ is the final coast time. Staging and coasting introduce discontinuities in mass and thrust, so the trajectory is split into successive flight arcs across which the solution is matched.

\subsubsection{Hamiltonian and Costate Equations}

Applying the general Hamiltonian definition to the ascent equations of motion~\eqref{eq:planar_eom} gives, for each flight arc~\cite{BettsSurvey1998}:
\begin{equation}
H = L + \boldsymbol{\lambda}^{T}\mathbf{f}
  = \lambda_s\,\dot{s} + \lambda_r\,\dot{r} + \lambda_v\,\dot{v} + \lambda_\gamma\,\dot{\gamma} \ ,
\label{eq:indirect_hamiltonian}
\end{equation}
where $\lambda_s,\lambda_r,\lambda_v,\lambda_\gamma$ are the adjoint (costate) variables conjugate to the state equations $\dot{s},\dot{r},\dot{v},\dot{\gamma}$ of~\eqref{eq:planar_eom}. The adjoint differential equations follow from
\begin{equation}
\frac{d\boldsymbol{\lambda}}{dt} = -\left(\frac{\partial H}{\partial \mathbf{x}}\right)^{T} \ ,
\label{eq:indirect_adjoint}
\end{equation}
which, for the drag-free free-flight dynamics, yields $\dot{\lambda}_s = 0$ (so that $\lambda_s = 0$, as also implied by the transversality conditions) together with~\cite{pontryagin1987mathematical}:
\begin{subequations}\label{eq:indirect_costates}
\begin{align}
\dot{\lambda}_r &= \frac{v\cos\gamma}{r^{2}}\,\lambda_\gamma
  - \frac{2\mu}{r^{3}}\!\left(\lambda_v\sin\gamma + \frac{\lambda_\gamma\cos\gamma}{v}\right), \label{eq:costate_r}\\[4pt]
\dot{\lambda}_v &= -\lambda_r\sin\gamma
  - \lambda_\gamma\!\left[\cos\gamma\!\left(\frac{1}{r} + \frac{\mu}{r^{2}v^{2}}\right) - \frac{T}{m}\frac{\sin\alpha}{v^{2}}\right], \label{eq:costate_v}\\[4pt]
\dot{\lambda}_\gamma &= -v\,\lambda_r\cos\gamma
  + \frac{\mu\,\lambda_v\cos\gamma}{r^{2}}
  + \lambda_\gamma\sin\gamma\!\left(\frac{v}{r} - \frac{\mu}{r^{2}v}\right), \label{eq:costate_gamma}
\end{align}
\end{subequations}
where $\mu$ is the Earth's gravitational parameter and $T$ the thrust (which vanishes on coast arcs), and the radial distance $r = R_E + h$ has been used in place of the altitude. Since the costate equations~\eqref{eq:indirect_costates} are homogeneous in $\boldsymbol{\lambda}$, the trajectory depends only on the \emph{direction} of the costate vector; the unknown initial costates can therefore be sought in the normalised interval $[-1,1]$, which reduces the search space.

\subsubsection{Optimal Control Law}

The optimal control minimises the Hamiltonian, i.e. $(\partial H/\partial u)^{T}=0$, subject to the Legendre--Clebsch condition requiring $\partial^{2}H/\partial u^{2}$ to be positive semi-definite. The single control is the angle of attack $\alpha$, assumed equal to the thrust-deflection angle~\cite{pallone2016}, so that $\alpha = \arg\min_\alpha H$. Carrying out the minimisation yields
\begin{equation}
\frac{\lambda_\gamma}{v}\sin\alpha + \lambda_v\cos\alpha = 0 \ ,
\label{eq:indirect_control_implicit}
\end{equation}
whose solution is
\begin{equation}
\sin\alpha = -\frac{\lambda_\gamma/v}{\sqrt{\left(\lambda_\gamma/v\right)^{2} + \lambda_v^{2}}} \ ,
\qquad
\cos\alpha = -\frac{\lambda_v}{\sqrt{\left(\lambda_\gamma/v\right)^{2} + \lambda_v^{2}}} \ ,
\label{eq:indirect_control}
\end{equation}
or, equivalently, $\alpha = \operatorname{atan2}\!\left(-\lambda_\gamma/v,\,-\lambda_v\right)$. The commanded thrust direction is thus determined entirely by the instantaneous costates and speed.

\subsubsection{Transversality and Boundary Conditions}

For a free final time the transversality condition reads $(\Phi_t + H)|_{t_f}=0$, with the endpoint function $\Phi = J + \boldsymbol{\nu}^{T}\boldsymbol{\Psi}(\mathbf{x}_f)$, where $\boldsymbol{\nu}$ are the time-independent adjoint variables conjugate to the boundary conditions. The vehicle must reach the target orbit with prescribed altitude $h'$, velocity $v'$, and flight-path angle $\gamma'$, giving the terminal boundary constraints
\begin{equation}
\boldsymbol{\Psi} =
\begin{bmatrix} h_f - h' \\ v_f - v' \\ \gamma_f - \gamma' \end{bmatrix} = \mathbf{0} \ .
\label{eq:indirect_terminal}
\end{equation}
By the Weierstrass--Erdmann corner conditions the costates remain continuous across successive flight arcs. Because the final time $t_f$ and final downrange are not prescribed---the coast and last-stage burn durations being themselves optimised---the transversality condition takes the form~\cite{pontani2014particle}:
\begin{equation}
H_f^{\text{last}} + H_f^{\text{coast}} - H_0^{\text{last}} = 0 \ ,
\qquad H_f^{\text{last}} < 0 \ ,
\label{eq:indirect_transversality}
\end{equation}
which relates the Hamiltonian at the end of the last thrust arc, the end of the coast arc, and the start of the last thrust arc.

\subsubsection{Penalty-Based Reformulation}

Rather than solving the resulting two-point boundary value problem by classical shooting, the boundary and optimality conditions are enforced by recasting it as a penalised minimisation suited to a population-based optimiser~\cite{Morgado2022_MultistagePrelimDesignTrajectoryOpt}: the terminal altitude, velocity, and flight-path-angle errors, together with the transversality residual~\eqref{eq:indirect_transversality}, are gathered into a single augmented objective whose remaining unknowns---the initial costates and the arc-timing parameters---are searched by a particle swarm optimiser. This reformulation trades the ill-conditioned initial-costate guess of classical shooting for a robust global search, at the cost of only approximate satisfaction of the optimality conditions. The concrete augmented objective, its penalty weights, the decision vector, and the multi-arc integration used to evaluate it are detailed in the methodology (Section~\ref{sssec:indirect_pmp}).
...
\end{equation}
where $\alpha^{\star}$ denotes the unconstrained value given by Eq.~\eqref{eq:indirect_control} and $\alpha_{\max}$ is a prescribed bound. The constraint is enforced only while the vehicle remains within the atmosphere and is released once an atmosphere-exit criterion is met, so that the near-vacuum arc recovers the exact interior control~\eqref{eq:indirect_control}~\cite{gath2001optimization}. The specific exit criterion used---shared with the fairing-jettison and guidance-activation logic---is described in the methodology (Section~\ref{sssec:indirect_pmp}).
...
\end{equation}
that neither alters the trajectory nor enlarges the set of unknowns. With no terminal mass cost the transversality condition is $\lambda_m(t_f)=0$; because the quadrature is passive, this condition is imposed \emph{a posteriori} on the integrated costate history, as detailed in the methodology (Section~\ref{sssec:indirect_pmp}). Including $\lambda_m$ renders the Hamiltonian constant along each powered arc, confirming that the multi-arc solution is a consistent extremal of the full-ascent problem~\cite{pontani2013ascent,pontani2019ascent}.
...
The models introduced in this chapter, the equations of motion, the guidance laws, and the optimal-control formulation, must ultimately be solved numerically. This section summarizes the numerical building blocks used in this work: the integration of the equations of motion with event handling, the root-finding and iteration schemes underlying several guidance laws, and the optimization algorithms used to shape and refine the trajectory.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Numerical Integration of the Equations of Motion}
\label{ssec:integration}

The equations of motion~\eqref{eq:planar_eom} form a system of non-linear ordinary differential equations (ODEs) with no closed-form solution, and must therefore be integrated numerically. Explicit Runge--Kutta methods are the standard choice for such non-stiff initial-value problems, advancing the state through a weighted combination of derivative evaluations (stages) within each step~\cite{burden2015numerical}. For a state $\mathbf{x}$ governed by $\dot{\mathbf{x}}=\mathbf{f}(t,\mathbf{x})$, a generic $s$-stage method advances the solution as
\begin{equation}
\mathbf{x}_{n+1} = \mathbf{x}_n + h\sum_{i=1}^{s} b_i\,\mathbf{k}_i \ ,
\qquad
\mathbf{k}_i = \mathbf{f}\!\left(t_n+c_i h,\; \mathbf{x}_n + h\sum_{j=1}^{i-1} a_{ij}\,\mathbf{k}_j\right) \ ,
\label{eq:rk_generic}
\end{equation}
where $h$ is the step size and the coefficients $\{a_{ij},b_i,c_i\}$ define the particular scheme.

In this work the Dormand--Prince RK4(5) pair is adopted~\cite{dormand1980family}. This embedded scheme computes two solutions, of orders four and five, from the same stage evaluations; their difference provides a local truncation-error estimate that is used to control the step size adaptively, taking large steps in smooth coasting arcs and small steps where the dynamics change rapidly (for example near staging or maximum dynamic pressure)~\cite{burden2015numerical}. The implementation used is that provided by the \texttt{SciPy} scientific-computing library~\cite{virtanen2020scipy}.

A distinctive requirement of ascent simulation is the precise detection of \emph{events}: the instants at which the dynamics change qualitatively (main-engine cut-off, stage separation, atmosphere exit, orbital-insertion cut-off). Each event is expressed as a scalar function $g(t,\mathbf{x})$ whose sign change marks the transition; once the integrator brackets a sign change, the exact crossing time is located by a root-finding method, typically Brent's method, which combines the guaranteed convergence of bisection with the speed of inverse quadratic interpolation~\cite{brent1973algorithms}. The integration is then restarted with the updated configuration, allowing the mission phases to be chained automatically without prescribing their durations in advance.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Root-Finding and Iterative Solution}
\label{ssec:rootfinding}

Several guidance laws require the solution of non-linear algebraic relations at run time. The classical Powered Explicit Guidance scheme, for instance, determines its steering parameters through a predictor--corrector loop that is, in essence, a fixed-point iteration on the time-to-go: each pass recomputes the thrust integrals and updates $t_{go}$ until successive estimates agree to within a tolerance~\cite{burden2015numerical}. Because such fixed-point iterations may converge slowly or oscillate, a relaxation factor is often introduced, blending the new and previous estimates (successive over- or under-relaxation) to stabilize and accelerate convergence~\cite{burden2015numerical}.

A second use of root-finding arises when an open-loop steering law is fixed by enforcing terminal constraints, as in the exponential pitch law: a Newton-type solver adjusts the law parameters until a simplified forward propagation meets the desired terminal radius and flight-path angle. This is a \emph{single-shooting} strategy, the simplest of the boundary-value-problem methods, and the same idea underlies the indirect optimal-control approach discussed earlier in this chapter, where the unknowns are the initial co-states rather than steering parameters~\cite{BettsSurvey1998}.
...
The trajectory-shaping problem, choosing the kick angle, the coast and burn timing, or the co-state initial values that yield the best ascent, is solved here by gradient-free global optimization. This choice is motivated by the event-driven nature of the simulation: staging and cut-off events introduce discontinuities in the objective that would defeat gradient-based optimizers. Two complementary strategies are used.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Brute-Force Grid Search}

When the design space is low-dimensional, an exhaustive grid search provides a simple and robust optimizer: the objective is evaluated at a large number of points spanning the admissible interval of each design variable, and the best is retained. Although its cost grows exponentially with the number of variables, for a single design variable it is highly effective and, crucially, immune to the non-smoothness introduced by discrete events that would mislead derivative-based methods. It also guarantees the global optimum to within the grid resolution.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Particle Swarm Optimization}

For higher-dimensional design spaces, Particle Swarm Optimization (PSO) is employed~\cite{kennedy1995particle}. PSO is a population-based metaheuristic inspired by the collective motion of bird flocks: a swarm of candidate solutions (particles) explores the design space, each particle remembering its own best position and being attracted toward both that personal best and the best position found by the entire swarm. The velocity and position of particle $i$ are updated as~\cite{kennedy1995particle, shi1998modified}:
\begin{equation}
\begin{aligned}
\mathbf{v}_i &\leftarrow \omega\,\mathbf{v}_i + c_1 r_1\bigl(\mathbf{p}_i-\mathbf{x}_i\bigr) + c_2 r_2\bigl(\mathbf{g}-\mathbf{x}_i\bigr) \ ,\\
\mathbf{x}_i &\leftarrow \mathbf{x}_i + \mathbf{v}_i \ ,
\end{aligned}
\label{eq:pso_update}
\end{equation}
where $\mathbf{p}_i$ is the particle's personal-best position, $\mathbf{g}$ the swarm's global-best position, and $r_1,r_2$ are uniform random numbers in $[0,1]$. The inertia weight $\omega$ together with the cognitive and social coefficients $c_1$ and $c_2$ balance exploration of the design space against exploitation of known good regions~\cite{shi1998modified}. PSO requires no gradient information, handles non-smooth and multimodal objectives naturally, and is therefore well suited to the event-driven ascent objectives considered here. The implementation relies on the \texttt{PyGMO} optimization library~\cite{biscani2020pagmo}.

In the indirect optimisation of Section~\ref{ssec:indirect_ascent}, PSO plays the specific role of supplying the initial costates and arc-timing parameters that the indirect method cannot initialise on its own, while minimising the propellant required for the mission~\cite{pontani2014particle}. The precise decision vector encoded by each particle, and the augmented objective it is scored against, are given in the methodology (Section~\ref{sssec:indirect_pmp}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Constrained Optimization via Penalty Functions}

The ascent objectives are subject to constraints, such as reaching the target altitude, velocity, and flight-path angle, that the gradient-free optimizers above do not enforce directly. These are incorporated through the penalty-function method, in which constraint violations are added to the objective weighted by penalty coefficients, so that infeasible candidates are scored poorly and the search is driven toward the feasible region~\cite{nocedal2006numerical}. A large fixed penalty is also assigned to non-physical outcomes (for example trajectories that crash or deplete their propellant before insertion), effectively removing them from consideration. The choice of penalty weights trades constraint satisfaction against the primary objective and must be tuned for the problem at hand~\cite{nocedal2006numerical}.
