...
introduced in Section~\ref{ssec:optimization}: terminal errors and infeasibility
are folded into a single scalar to be minimised.
...
inter-stage coast, and the upper-stage thrust--coast--thrust arcs. On the
first-stage arc the engine is modelled with a constant representative thrust and
specific impulse (the mean of the sea-level and vacuum values), a first-version
simplification of the altitude-varying Stage-1 performance. The costates are
...
single \texttt{GUIDANCE\_MODE} and flies an ordered schedule of laws, each
activated at a prescribed altitude. Gravity turn is the implicit prefix from
lift-off to the first activation altitude; every non-final segment then steers
toward the optimal $(h,v,\gamma)$ waypoint taken from a cached indirect-PMP
reference (Section~\ref{sssec:indirect_pmp}) at the next activation altitude, and
the final segment performs the orbit insertion. Insertion reuses the
\texttt{pso\_coast} thrust--coast--thrust engine and its four-variable design
vector of Eq.~\eqref{eq:pso_coast_vector}; the activation altitudes are
user-specified.
...
sourced from the reference timing. The supported segment laws are \texttt{apollo},
\texttt{peg\_new}, \texttt{linear\_tangent}, \texttt{bilinear\_tangent}, and
\texttt{indirect\_pmp} (which replays the stored optimal $\alpha$); the two tangent
laws are angle-only, matching a waypoint's flight-path angle but not its altitude
or speed, so the final segment carries the insertion.
