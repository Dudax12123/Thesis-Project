...
Each phase is propagated with the Dormand--Prince RK4(5) scheme of
Section~\ref{sec:numerical_methods}, as provided by
\texttt{scipy.integrate.solve\_ivp}~\cite{virtanen2020scipy}, using adaptive
step-size control, a tight tolerance, a bounded maximum step, and dense output for
post-processing. Phase transitions are located by \emph{event functions} --- scalar
functions $g(t,\mathbf{x})$ whose sign change marks a transition. When the
integrator brackets a zero-crossing, the exact crossing time is found by
root-finding (Brent's method; Section~\ref{ssec:rootfinding}) and the phase is
terminated. Events include propellant depletion
...
$F_D$ is aerodynamic drag, $F_L$ is aerodynamic lift,
...
pressure is $q=\tfrac12\rho v^{2}$. Three criteria are available for declaring
atmosphere exit and activating exo-atmospheric guidance: an altitude threshold
($h>65$~km~\cite{kluever2018space}), a dynamic-pressure threshold
($q<1000$~Pa, the default), or an aerothermal-flux threshold
($\tfrac12\rho v^{3}<1135~\mathrm{W\,m^{-2}}$). The selected criterion is a
simulation parameter (\texttt{ATMOSPHERE\_EXIT\_METHOD}) and is shared with the
fairing-jettison logic.
...
Eqs.~\eqref{eq:cd} and~\eqref{eq:q_dyn_press}. Aerodynamic lift is included on the
same basis, $F_L = q\,C_L\,A$ with a constant lift coefficient $C_L=0.1$ (the $F_L$
term of Eq.~\eqref{eq:sim_gammadot}, enabled by the default \texttt{INCLUDE\_LIFT}).
...
rocket-equation and a PEG-based time-to-go estimate. Besides being selected
individually, several of these laws can also be chained by altitude within a single
ascent through the segmented mode of Section~\ref{ssec:segmented}. The
atmospheric-arc theory of each law is developed in
Chapter~\ref{chapter:theoretical_background}; this section documents the
implemented form.
...
The triangular profile of Eq.~\eqref{eq:kick_profile} (selected by
\texttt{KICK\_PROFILE\_MODE}) is applied only in the brute-force
\texttt{apogee\_check} architecture. The three PSO architectures instead impose an
\emph{instantaneous} flight-path-angle jump at the end of the vertical rise,
$\gamma \to \gamma_p$ (equivalently a kick angle $\gamma_p-\pi/2$), so under those
paths \texttt{KICK\_PROFILE\_MODE} has no effect and $\gamma_p$ alone parametrises
the manoeuvre.
...
The classical PEG mode maintains the linear tangent-steering program
$\sin(\text{pitch}[t]) = A + B\,t$ --- a scalar realisation of the explicit-guidance
principle derived in Section~\ref{sssec:peg_theory} --- and updates the steering
constants $A,B$ and the burn-time estimate $T$ each major cycle. Given the rocket-equation thrust
...
The \texttt{peg\_new} mode implements the velocity-to-be-gained formulation --- the
vector Powered Explicit Guidance form derived in
Section~\ref{sssec:peg_theory} --- in
...
converge the current-state initial guess is retained. Under the \texttt{pso\_coast}
architecture the shooting solve is bypassed: $(a,b)$ are instead carried as two
additional PSO decision variables and re-epoched at the start of each burn arc, so
the coefficients are chosen by the outer optimiser rather than by \texttt{fsolve}.
This is the simplest boundary-value method, and the same shooting idea underlies
the indirect optimal-control architecture, where the unknowns are the initial
costates rather than pitch coefficients.
...
\texttt{COAST\_METHOD} together with the \texttt{indirect\_pmp} mode; a further
\emph{segmented} mode (Section~\ref{ssec:segmented}) chains several laws within a
single ascent. All share the non-dimensional penalised-objective philosophy
introduced in Section~\ref{ssec:optimization}: terminal errors are folded into a
single scalar to be minimised, and any non-physical outcome---a crash or propellant
depletion before insertion---is driven out of contention by a large fixed crash
penalty $C=10^{20}$.
...
Eq.~\eqref{eq:indirect_transversality}, and $C$ is the shared crash penalty
introduced at the start of this section. Minimising $J'$ drives the swarm towards
feasible extremals; the run uses a swarm of $250$ particles over $500$ generations.

\paragraph{Multi-arc integration.}
The objective~\eqref{eq:indirect_augmented} is evaluated by integrating the
trajectory in two phases. The first stage flies the fixed gravity turn produced by
the shared Stage-1 routine --- the pitch-over kick followed by an uncontrolled
$\alpha\equiv0$ arc up to main-engine cut-off and staging; no costates are carried
here, since the indirect control acts only on the exo-atmospheric flight after
fairing jettison, where aerodynamic forces are negligible. After the inert-mass
staging drop and a short ballistic coast to second-stage ignition, the augmented
state--costate system is initialised: the three costates
$[\lambda_r,\lambda_v,\lambda_\gamma]$ encoded by the decision vector are seeded at
ignition and propagated alongside the reduced state $[s,r,v,\gamma,m]$ under the
costate-optimal control of Eq.~\eqref{eq:indirect_control}. The upper stage is then
integrated as a thrust--coast--thrust sequence (an initial burn, a coast, and a
final burn to insertion), across which the costates and the Hamiltonian remain
continuous by the Weierstrass--Erdmann corner conditions, so no interior jump
conditions are imposed. The terminal errors and the transversality residual of the
resulting extremal are then scored through Eq.~\eqref{eq:indirect_augmented}.

\subsection{Segmented Multi-Law Guidance}
\label{ssec:segmented}

The architectures above fly a single guidance law for the whole guided ascent. An
optional segmented mode (\texttt{MULTI\_GUIDANCE\_ENABLED}) instead ignores the
single \texttt{GUIDANCE\_MODE} and flies an ordered schedule of laws, each activated
at a prescribed altitude. The first scheduled law takes over immediately after the
pitch-over kick; a gravity turn is simply one of the selectable laws rather than a
forced prefix, so the study can compare an active law flown from lift-off against a
passive gravity-turn lead-in. Every non-final segment steers toward the optimal
$(h,v,\gamma)$ waypoint read from a cached indirect-PMP reference
(Section~\ref{sssec:indirect_pmp}) at the next activation altitude, and the final
segment performs the orbit insertion, reusing the \texttt{pso\_coast}
thrust--coast--thrust engine and its four-variable design vector of
Eq.~\eqref{eq:pso_coast_vector}. Because that reference is the Stage-2-only
extremal, its atmospheric portion is the reference gravity turn, so any waypoint
below second-stage ignition tracks that arc rather than an optimised atmospheric
control.

The distinctive feature is that, because a segment can activate below MECO, a
closed-loop law can fly \emph{during} the first stage --- something the single-law
modes cannot do, since they gate active guidance on second-stage ignition. To keep
the time-to-go--dependent laws (Apollo and the tangent laws) counting down
monotonically across staging rather than saturating at the stage boundary, the
segmented mode replaces the rocket-equation $t_{go}$ estimate with a
planned-deadline countdown $t_{go}=t_{\mathrm{deadline}}-t$, the deadlines being
sourced from the reference timing. The supported segment laws are
\texttt{gravity\_turn}, \texttt{apollo}, \texttt{peg\_new}, \texttt{linear\_tangent},
\texttt{bilinear\_tangent}, and \texttt{indirect\_pmp} (which replays the stored
optimal~$\alpha$); the two tangent laws are angle-only, matching a waypoint's
flight-path angle but not its altitude or speed, so the final segment carries the
insertion.

The activation altitudes may be fixed by the user or, when
\texttt{MULTI\_GUIDANCE\_OPTIMIZE\_ALTITUDES} is enabled, optimised jointly with the
insertion parameters. In that case the design vector of
Eq.~\eqref{eq:pso_coast_vector} is augmented with the $(n-1)$ activation altitudes
of the schedule --- every segment except the first, which stays anchored at the kick
--- appended as normalised fractions that are mapped cumulatively to a strictly
increasing altitude sequence within prescribed bounds, so that every particle yields
an admissible ordering without rejection. Optimising the hand-off altitudes lets the
swarm trade the segments against one another to minimise the second-stage burn time
subject to a clean insertion, the same objective minimised by the single-law
architectures. The segmented solver is driven by its own dedicated swarm settings
(\texttt{PSO\_MG\_*}), decoupled from those of \texttt{pso\_coast}.

\subsection{Separation of Concerns}
\label{ssec:separation}

This guidance-agnostic decoupling of the outer optimiser from the inner
simulation, introduced in Section~\ref{sec:architecture} and shared by every
architecture above, is what makes the comparative study fair: the same trajectory
engine, environment models, and scoring normalisation are applied to every guidance
law, so differences in the results reflect the laws themselves rather than the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l l}
    ...
    Atmosphere / aero       & \texttt{INCLUDE\_DRAG}, \texttt{INCLUDE\_LIFT},
                              ...
                              (altitude / dynamic pressure / aerothermal flux) \\
    Kick profile            & \texttt{KICK\_PROFILE\_MODE} (triangular / instantaneous;
                              triangular honoured only under \texttt{apogee\_check}) \\
...
Maximum Thrust (SL)      & 7{,}607 kN    & --       \\
Maximum Thrust (V)       & 8{,}227 kN    & 981 kN   \\
