...
def _configure(monkeypatch, case_name, **extra):
    ...
    # peg_direct and the segmented cases were measured with swarm-timed burns, which
    # the matrix replaced by law-terminated ones on 2026-09-25 (those burns refresh
    # outside the ODE in either mode, so there would be nothing to compare)
    pinned = {"peg_direct": {"DIRECT_LAW_TERMINATED_CUTOFF": False, "DIRECT_OPTIMIZER": "pso"},
              "show_seg_fixed_alt": {"SEGMENTED_LAW_TERMINATED_ARCS": False},
              }.get(case_name, {})
