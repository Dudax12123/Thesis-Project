...
| `PEG_MAJOR_LOOP_RATE` (L159) | float s | `2.0` | PEG major-loop recompute period (peg: A,B,T; peg_new: v_go, t_go, λ'_r). | peg, peg_new. |
...
- **peg_new and apollo refresh their coefficients inside the ODE right-hand side under `pso_coast`,
  ...
  2026-09-24; the config default stays `"in_rhs"`, and the results matrix flies `"cycle"` since
  2026-09-25.** The refresh fires whenever `PEG_MAJOR_LOOP_RATE` /
