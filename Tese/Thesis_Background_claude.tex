...
The indirect framework introduced above is now specialised to the powered-ascent problem addressed in this work, following the formulation of~\cite{Morgado2022_MultistagePrelimDesignTrajectoryOpt}. Direct methods are generally more robust in convergence, even with poor initial guesses, whereas the indirect method is harder to initialise but yields more accurate results, a property that is critical for aerospace applications~\cite{conway2010spacecraft}. The indirect control law is applied only on the free-flight (exo-atmospheric) arcs, after fairing jettison, where aerodynamic forces are negligible; during the preceding atmospheric flight the vehicle performs an uncontrolled gravity turn following the initial pitch manoeuvre.
