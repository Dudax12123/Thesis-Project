...
\end{equation}
The Coriolis term depends on the vehicle velocity and deflects the trajectory laterally, coupling the in-plane motion to the cross-range direction, whereas the centrifugal term depends only on position and acts to reduce the effective gravity, most strongly near the equator~\cite{curtis2020orbital}. When projected onto the local East--North--Up directions and then onto the flight-path frame, these accelerations introduce additive corrections to the velocity and flight-path-angle rates of Eqs.~\eqref{eq:planar_eom_a}--\eqref{eq:planar_eom_b}; their detailed projection is presented in the methodology chapter. The centrifugal contribution is small but non-negligible, while the Coriolis contribution is the dominant rotational effect on the achieved orbital inclination~\cite{vallado2013fundamentals}.
...
Rather than solving the resulting two-point boundary value problem by classical shooting, the boundary and optimality conditions are enforced through a static penalty function, transforming the problem into an unconstrained minimisation suited to a population-based optimiser~\cite{Morgado2022_MultistagePrelimDesignTrajectoryOpt}. The augmented objective is
\begin{equation}
J' = J + \sum_{c=1}^{3} s_c\,\bigl\lVert x_{c,f} - x_{c}'\bigr\rVert
   + s_4\,\bigl\lVert H_f^{\text{last}} + H_f^{\text{coast}} - H_0^{\text{last}}\bigr\rVert
   + C \ ,
\label{eq:indirect_augmented}
\end{equation}
where the first sum penalises the terminal altitude, velocity, and flight-path-angle errors (with constraint weight factors $s_c$), the fourth term penalises any violation of the transversality condition~\eqref{eq:indirect_transversality}, and $C$ is a large fixed penalty applied to non-physical trajectories,
\begin{equation}
C =
\begin{cases}
0, & \text{trajectory constraints satisfied}, \\[2pt]
10^{20}, & \text{trajectory constraints violated} .
\end{cases}
\label{eq:indirect_penalty}
\end{equation}
Minimising $J'$ therefore drives the optimiser implicitly toward feasible trajectories that also satisfy the optimality conditions. The remaining unknowns---the initial costates and the arc-timing parameters---are determined by the particle swarm optimiser described in Section~\ref{ssec:optimization}.
...
In the indirect optimisation of Section~\ref{ssec:indirect_ascent}, PSO plays the specific role of supplying the parameters that the indirect method cannot initialise on its own, while minimising the propellant required for the mission~\cite{pontani2014particle}. Each particle encodes a candidate parameter set comprising the unknown initial costates $(\lambda_{0r},\lambda_{0v},\lambda_{0\gamma})$ together with the coast-phase duration $\Delta t_c$, the coast start (initialisation) time, the last-stage burn duration $\Delta t_T$, and the pitch-manoeuvre angle $\gamma_p$. For each particle a complete trajectory is simulated until the vehicle reaches orbit or a maximum number of iterations is exceeded, and the augmented objective~\eqref{eq:indirect_augmented} is evaluated; the swarm then evolves toward the parameter set that minimises propellant consumption while satisfying the optimality conditions.
